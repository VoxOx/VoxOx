...
//		if ( purple_account_get_bool(js->gc->account, "use_tls", TRUE) )		//JRT-XXX //VOXOX - JRT - 2009.05.10 - TODO: complete testing.
		{
...
void jabber_close(PurpleConnection *gc)
{
	...
	if ( js == NULL )		//VOXOX - JRT - 2009.10.12 - Prevent crash when network is lost or resuming from hibernation.
		return;
	...
	if(js->iq_callbacks)
	{
		...
		js->iq_callbacks = NULL;	//VOXOX - JRT - 2009.10.13 - Fix crash.
	}

	if(js->disco_callbacks)
	{
		g_hash_table_destroy(js->disco_callbacks);
		js->disco_callbacks = NULL;	//VOXOX - JRT - 2009.10.13 - Fix crash.
	}

	if(js->buddies)
	{
		g_hash_table_destroy(js->buddies);
		js->buddies = NULL;			//VOXOX - JRT - 2009.10.13 - Fix crash.
	}

	if(js->chats)
	{
		g_hash_table_destroy(js->chats);
		js->chats = NULL;			//VOXOX - JRT - 2009.10.13 - Fix crash.
	}
