...
int skype_send_im(PurpleConnection *gc, const gchar *who, const gchar *message, PurpleMessageFlags flags);
 //VOXOX CHANGE CJC ADD METHOD TO CALL CONTACT
static void skype_buddy_call_voxox(PurpleConnection *gc, const char *info); //VOXOX CHANGE ASV 04-16-2009: added static to the beginning of the function declaration.
...
PurplePluginProtocolInfo prpl_info = {
	...
	//VOXOX CHANGE CJC, METHOD WAS NULL, CHANTE IT TO BE ABBLE TO CALL SKYPE CONTACTS FROM VOXOX, NOT THE PRITTIES WAY, BUT WORKS
	skype_buddy_call_voxox,/* set_info */
...
#endif
//VOXOX CHANCE CJC CALL BUDDY FROM VOXOX
static void skype_buddy_call_voxox(PurpleConnection * gc, const char *info)
{
	skype_send_message_nowait("CALL %s", info);	
}
...
skype_normalize(const PurpleAccount *acct, const char *who)
{
	...
	//VOXOX - JRT - 2009.10.07 - We get a lot of crashes here when user has changed networks (like moving from home to office).
	//							 It looks like there is no way for the g_free to have bad data, but it has already been free'd.
	//							 So, we will set ptr to NULL, so avoid.
	if (last_normalize)
	{
		g_free(last_normalize);
		last_normalize = NULL;		//VOXOX - JRT - 2009.10.07 
	}
...
#ifdef __APPLE__
	reply = skype_send_message("NAME VoxOx"); //VOXOX CHANGE Alexander 04-13-09
#else
