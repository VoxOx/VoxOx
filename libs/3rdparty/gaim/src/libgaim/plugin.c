...
gaim_plugin_probe(const char *filename)
{
	...
	/* 
 	 ...
 	 * Not having this check caused gaim_plugin_unload to
 	 * enter an infinite loop in certain situations by passing
 	 * gaim_find_plugin_by_id a NULL value. -- ecoffey
 	 */
	if (!plugin->info->id || !strcmp(plugin->info->id, ""))
	{
		plugin->error = g_strdup_printf(_("This plugin has not defined an ID."));
		gaim_debug_error("plugins", "%s is not loadable: info->id is not defined.\n", plugin->path);
		plugin->unloadable = TRUE;
		return plugin;
	}
