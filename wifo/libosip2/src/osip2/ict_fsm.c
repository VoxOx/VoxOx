...
ict_create_ack (osip_transaction_t * ict, osip_message_t * response)
{
  ...
  /* should be used instead?? ......May be not..... */
  {
	...
	// Wengo : use the record-routes contained in the latest response instead of the original ones
	while (!osip_list_eol (&ict->last_response->record_routes, pos))
      {
        orig_route =
          (osip_route_t *) osip_list_get (&ict->last_response->record_routes, pos);
        osip_route_clone (orig_route, &route);
        osip_list_add (&ack->routes, route, 0);
        pos++;
      }
  }
