...
QtUMEmail::QtUMEmail(CChatHandler & cChatHandler,
...
void QtUMEmail::setIMChatSession(IMChatSession * imChatSession) {
	
	_imChatSession = imChatSession;

}

void QtUMEmail::buildUI(){
...
void QtUMEmail::init(const QString & nickname,const QString & senderName,const QString & contactId,const QString & email,const QString & from){
...
void QtUMEmail::updateTranslationUI(){
...
void QtUMEmail::closeSession() {
	_imChatSession->close();
	
}

void QtUMEmail::changeFont() {
	bool ok;
...
void QtUMEmail::loadLastChatHistory(QString contactId){
	//Config & config = ConfigManager::getInstance().getCurrentConfig();
...
void QtUMEmail::changeFontColor() {
	bool ok;
	QFont font = QFontDialog::getFont(&ok, _chatEdit->currentFont(), _widget);
	if (ok) {
		_currentFont = font;
...
void QtUMEmail::setVisible(bool visible) {
	if (visible) {
		_chatEdit->setFocus();
	}
}
...
void QtUMEmail::addToHistory(const QString & contactId, const QString & senderName, const QString & protocolName, const QString & str, const QTime & time, const QDate & date) {
	QTime tmp;
	if (time.isNull()) {
...
void QtUMEmail::addToHistoryTranslation(const QString & contactId, const QString & senderName, const QString & protocolName, const QString & originalMessage,const QString & translatedMessage, const QTime & time, const QDate & date) {
	QTime tmp;
	if (time.isNull()) {
		tmp = QTime::currentTime();
	} else {
...
void QtUMEmail::addStatusMessage(const QString & statusMessage) {
	_ui->chatHistory->insertStatusMessage(statusMessage, QTime::currentTime());
}

void QtUMEmail::sendPressedSlot(){//VOXOX - CJC - 2010.01.18 
...
void QtUMEmail::sendMessage() {
	...
	text.replace("&", tr("and"));
	//Change userName
	_subject = QString("VoxOx Email from %1").arg(_nickName);

	QString message = QtChatUtils::encodeMessageChatToEmail(getEmail(),getFrom(),_subject,text);
	addToHistory("self", _nickName,"Email", text);
...
void QtUMEmail::sendTranslatedMessage() {//VOXOX - CJC - 2010.01.18 
	...
	_translation->translateAndSendMessage(QString("self"),_nickName,"Email",text);
...
void QtUMEmail::processRecievedMessage(const QString & contactId, const QString & senderName, const QString & protocolName,const QString & originalText){
...
void QtUMEmail::translateMessageSlot(const QString & contactId, const QString & senderName, const QString & protocolName,const QString & originalText,const QString & translatedText, bool success, bool sendMessageToSession){
	...
	if(success){
		...
		if(sendMessageToSession){
			...
			text.replace("&", tr("and"));
			//Change userName
			_subject = QString("VoxOx Email from %1").arg(senderName);
			QString message = QtChatUtils::encodeMessageChatToEmail(getEmail(),getFrom(),_subject,text);
			_imChatSession->sendMessage(message.toUtf8().constData());
...
void QtUMEmail::translateErrorSlot(){
...
void QtUMEmail::updateAvatarFrame(const QString & contactId) {
...
void QtUMEmail::updateUserAvatar() {
...
void QtUMEmail::saveHistoryAsHtml() {
	_ui->chatHistory->saveHistoryAsHtml();
}

void QtUMEmail::setContactConnected(bool connected) 
{
...
QTextEdit * QtUMEmail::getChatEdit() {
	return _chatEdit;
}
...
bool QtUMEmail::eventFilter(QObject* object, QEvent* event) {
...
bool QtUMEmail::historyKeyPressEventFilter(QKeyEvent* event) {
...
bool QtUMEmail::editKeyPressEventFilter(QKeyEvent* event) {
	int key = event->key();
...
void QtUMEmail::translationClickedSlot(){
...
void QtUMEmail::translationSettingsSlot(QtEnumTranslationMode::Mode mode,const QtTranslationLanguage & source,const QtTranslationLanguage & dest){
...
CContactList& QtUMEmail::getCContactList()
{
