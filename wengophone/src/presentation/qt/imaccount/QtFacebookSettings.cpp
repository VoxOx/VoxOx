...
static const std::string MYSPACE_CREATE_NEW_ACCOUNT_LINK_EN = "http://www.facebook.com/";

QtFacebookSettings::QtFacebookSettings(UserProfile & userProfile, IMAccount & imAccount, QDialog * parent)
	: QtIMAccountPlugin(userProfile, imAccount, parent) {

	init();
}

QtFacebookSettings::~QtFacebookSettings() {
	OWSAFE_DELETE(_ui);
}

void QtFacebookSettings::init() {
	_IMSettingsWidget = new QWidget(_parentDialog);

	_ui = new Ui::FacebookSettings();
